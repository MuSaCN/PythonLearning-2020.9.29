...
def signalfunc(para):
    ...
    return result
# ---计算信号，不重复持仓
def signalfunc_NoRepeatHold(para):
    k = para[0]
    holding = para[1]
    lag_trade = para[2]
    trade_direct = para[3] # "BuyOnly","SellOnly","All"
...
if __name__ == '__main__':
    ...
    para_all = [(k, holding, lag_trade, "All") for k in range(1, k_end + 1) for holding in
                range(1, holding_end + 1) for lag_trade in range(1, lag_trade_end + 1)]
    # ---分析做多信号
    run(signalfunc,para_buyonly,"动量_Buy")
    # ---分析做空信号
    run(signalfunc,para_sellonly,"动量_Sell")
    # ---分析多空都做信号
    run(signalfunc,para_all,"动量_All")
